...
| App is silently killed on launch on Apple Silicon (`Killed: 9` in Terminal, or it bounces in the Dock and quits) | Mach-O code signature invalidated — `dylibbundler` rewrote the load commands, stripping the linker's ad-hoc signature | Re-run `./scripts/macos-bundle.sh`, which now ad-hoc signs every binary. To re-sign by hand: `codesign --force --sign - dhewm3.app/Contents/MacOS/dhewm3` (and each `*.dylib`). Verify with `codesign --verify --strict dhewm3.app/Contents/MacOS/dhewm3`. |
